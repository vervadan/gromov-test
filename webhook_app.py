...
def handle_tg_update(update):
    ...
    if text in ['/start', '🧪 Пройти тест'] or text.startswith('/start '):
        ...
        if text == '🧪 Пройти тест':
            ...
            send_tg_message(chat_id,
                'Привет!\n\n'
                'Я Алексей Громов — разбираю, почему деньги не задерживаются, даже когда их достаточно.\n\n'
                'Если ещё не проходил тест — нажми «🧪 Пройти тест». Узнаешь свой тип финансового мышления.\n\n'
                'Если уже оплатил разбор — нажми «📩 Получить файл».',
                MAIN_KEYBOARD)
